# ---------------------------------------------------------------------------- #
# Script : 10-2_posthoc_sPLS_lags_WII.R
...
M0 = compare_models_123(data=X, y_str="y", time_str="y_time", cov_str1=cov0_acc0, cov_str2=cov0_acc1, cov_str3=cov0_acc2, 
                         ...
                         folloup=quantile(X$y_time,probs=0.9), precision = 0.0001, round=4, do_sens=0.75)
...
setwd(dir = dir_res)
write.csv(M0, file = "m0_wii_lag.csv", row.names = TRUE)
write.csv(M1, file = "m1_wii_lag.csv", row.names = TRUE)
write.csv(M2, file = "m2_wii_lag.csv", row.names = TRUE)
write.csv(M3, file = "m3_wii_lag.csv", row.names = TRUE)

write.csv(H0, file = "h0_wii_lag.csv", row.names = TRUE)
write.csv(H1, file = "h1_wii_lag.csv", row.names = TRUE)
write.csv(H2, file = "h2_wii_lag.csv", row.names = TRUE)
write.csv(H3, file = "h3_wii_lag.csv", row.names = TRUE)
