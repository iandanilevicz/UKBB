# ---------------------------------------------------------------------------- #
# Script : 9-1_posthoc_sPLS_models_UKB.R
...
setwd(dir = dir_posthoc)
write.csv(M0, file = "m0_ukb_age_score_sex.csv", row.names = TRUE)
...
M1 = compare_models_123(data=X, y_str="y", time_str="y_time", 
                        ...
                        folloup=quantile(X$y_time,probs=0.9), precision = 0.001, round=8, do_sens=FALSE)
setwd(dir = dir_posthoc)
write.csv(M1, file = "m1_ukb_age_score_edu.csv", row.names = TRUE)
...
setwd(dir = dir_posthoc)
write.csv(M2, file = "m2_ukb_age_score_apoe.csv", row.names = TRUE)
